...
type Props = {
  ...
  clearReminderEffect: (todoId: number) => void; // ✅ NEW: Function to clear reminder effect
...
export default function TodoList({
  ...
  clearReminderEffect, // ✅ NEW: Destructure clearReminderEffect function
  removeTodo,
  toggleTodo,
  updateTodo,
  deleteAllTodos,
  ...
  // ✅ DEBUG: Log received reminded tasks
        ...
        {todosArray.length === 0 ? (
          ...
          <AnimatePresence>
            {todosArray.map((todo) => {
              ...
              if (shouldPulse) {
                console.log(
                  `🎯 [VISUAL] Task ${todo.id} should pulse - reminded: ${isReminded}`
                );
              }
                ...
                >
                  ...
                  <div className="p-4 lg:hidden">
                    ...
                    <div className="flex gap-2 ml-8">
                      {/* Clear Reminder Effect Button - Only show if reminded */}
                      {shouldPulse && (
                        <button
                          onClick={() => clearReminderEffect(todo.id)}
                          className="flex items-center gap-2 px-3 py-2 text-sm font-medium text-amber-600 hover:text-amber-700 hover:bg-amber-50 rounded-lg transition-colors min-h-[44px] flex-1 justify-center sm:flex-initial sm:min-h-0 border border-amber-200"
                          title="Clear reminder notification effect"
                        >
