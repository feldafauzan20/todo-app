...
export const useToast = () => {
  ...
  const addToast = useCallback((toast: Omit<Toast, "id">) => {
    ...
    console.log(`[TOAST DEBUG] addToast called with:`, newToast);

    setToasts((prev) => {
      const updatedToasts = [...prev, newToast];
      console.log(`[TOAST DEBUG] Updated toasts array:`, updatedToasts);
  ...
  const showReminder = useCallback(
    (title: string, message: string, action?: Toast["action"]) => {
      console.log(`[TOAST DEBUG] showReminder called with:`, {
        title,
        message,
        action,
      });
      ...
      // Play reminder sound (different from notification sound)
      if (typeof window !== "undefined") {
        import("@/services/NotificationService").then(
          ({ notificationService }) => {
            console.log(`[TOAST DEBUG] Playing reminder sound`);
            notificationService.playReminderSound(); // Use reminder sound
          }
        );
      }
