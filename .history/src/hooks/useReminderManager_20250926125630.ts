...
export const useReminderManager = (
  ...
  useEffect(() => {
    ...
    const scheduleReminders = () => {
      ...
      const now = new Date();
      console.log(
        `⏰ [EFFECT #${effectCallCount.current}] Current time:`,
        now.toISOString()
      );
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          console.log(
            ...
            }] Key exists in processed: ${processedKeys.current.has(
              reminderKey
            )}`
          );
          ...
          if (processedKeys.current.has(reminderKey)) {
            console.log(
              `⏭️ [EFFECT #${effectCallCount.current}] SKIPPING - Already processed: ${reminderKey}`
            );
            return;
          }
          ...
          console.log(
            `🕐 [EFFECT #${effectCallCount.current}] Parsed reminder time:`,
          ...
          if (timeUntilReminder <= -60000) {
            console.log(
              `⏭️ [EFFECT #${effectCallCount.current}] SKIPPING - Reminder too old for "${todo.text}"`
            );
            return;
          }
          ...
          if (timeUntilReminder > 0) {
            console.log(
              `✅ [EFFECT #${effectCallCount.current}] SCHEDULING - Adding key to processed: ${reminderKey}`
            );
            ...
            const timer = setTimeout(() => {
              ...
              console.log(
                `🚨 [TIMER-${todo.id}] Effect call that created this timer: #${effectCallCount.current}`
              );
          ...
          console.log(
            `⏭️ [EFFECT #${effectCallCount.current}] SKIPPING - No reminder or completed: ${todo.text}`
          );
        }
      });
    ...
    const showReminderNotification = async (todo: Todo, effectNum: number) => {
      try {
        ...
        console.log(
          `🔔 [NOTIFICATION-${
            todo.id
          }] showToast available: ${typeof showToast}`
        );
        ...
        if (showToast) {
          ...
          console.log(
            `🚫 [NOTIFICATION-${todo.id}] No showToast function available`
          );
        }
