      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            // Parse reminder time - assume it's in WIB timezone
            ...
            if (todo.reminder.includes('T') && (todo.reminder.includes('+') || todo.reminder.includes('Z'))) {
              ...
              // Treat as local WIB time and convert to UTC
              const localReminderTime = new Date(todo.reminder);
              reminderTime = new Date(localReminderTime.getTime() - (7 * 60 * 60 * 1000));
            }
