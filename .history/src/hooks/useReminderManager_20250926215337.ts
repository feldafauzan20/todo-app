...
export const useReminderManager = (
  ...
  console.log(`[REMINDER DEBUG] useReminderManager called with:`, {
  ...
  const clearReminderEffect = (todoId: number) => {
    console.log(
      `[REMINDER DEBUG] Manually clearing reminder effect for task ${todoId}`
    );
    ...
    setRemindedTasks((prev) => {
      ...
      console.log(
        `[REMINDER DEBUG] Updated remindedTasks:`,
        Array.from(newSet)
      );
      return newSet;
    });
  ...
  const addRemindedTask = (todoId: number) => {
    console.log(`[REMINDER DEBUG] Adding task ${todoId} to reminded state`);

    setRemindedTasks((prev) => {
      const newSet = new Set(prev).add(todoId);
      console.log(
        `[REMINDER DEBUG] Updated remindedTasks after adding:`,
        Array.from(newSet)
      );
      return newSet;
    });
  ...
  useEffect(() => {
    ...
    console.log(
      `[REMINDER DEBUG] useEffect triggered (${effectCallCount.current})`
    );
    ...
    const scheduleReminders = () => {
      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          console.log(
            `[REMINDER DEBUG] Processing todo ${todo.id} with reminder:`,
            todo.reminder
          );
          ...
          if (currentReminder && currentReminder !== todo.reminder) {
            console.log(
              `[REMINDER DEBUG] Reminder time changed for task ${todo.id}`
            );
          ...
          if (processedReminders.has(reminderKey)) {
            console.log(
              `[REMINDER DEBUG] Already processed reminder:`,
              reminderKey
            );
            return;
          }
          ...
          try {
            // SIMPLIFIED: Parse reminder time with timezone handling
            console.log(
              `[REMINDER DEBUG] Parsing reminder time:`,
              todo.reminder
            );

            // Simplified parsing - just use as is for now
            const utcTime = new Date(todo.reminder);

            console.log(
              `[REMINDER DEBUG] Parsed UTC time:`,
              utcTime.toISOString()
            );
            console.log(
              `[REMINDER DEBUG] Is valid date:`,
              !isNaN(utcTime.getTime())
            );
            ...
            if (delay > 0 && delay < 24 * 60 * 60 * 1000) {
              ...
              console.log(
                `[REMINDER DEBUG] Scheduling reminder for task ${todo.id} in ${delay}ms`
              );

              const timer = setTimeout(() => {
                console.log(
                  `[REMINDER DEBUG] REMINDER TRIGGERED for task ${todo.id}`
                );
                ...
                if (showToast) {
                  console.log(
                    `[REMINDER DEBUG] Showing toast for task ${todo.id}`
                  );
                  showToast("Reminder!", `Don't forget: ${todo.text}`, {
                    ...
                    onClick: () => {
                      ...
                      console.log(
                        `[REMINDER DEBUG] Mark done clicked for task ${todo.id}`
                      );
                    },
                  });
                } else {
              ...
              console.log(
                `[REMINDER DEBUG] Reminder is recent, showing immediately for task ${todo.id}`
              );
            ...
            console.error(
              `[REMINDER DEBUG] Error processing reminder for task ${todo.id}:`,
              error
            );
          }
        }
  ...
  console.log(
    `[REMINDER DEBUG] Hook return - remindedTasks:`,
    Array.from(remindedTasks)
  );
