...
export const useReminderManager = (
  ...
  const [recentlyEditedReminders, setRecentlyEditedReminders] = useState<Set<number>>(new Set());
  ...
  const addRemindedTask = (todoId: number) => {
    ...
    console.log(`🎯 Removed task ${todoId} from recently edited reminders (reminder triggered)`);
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          if (currentReminder && currentReminder !== todo.reminder) {
            ...
            console.log(`🎯 Added task ${todo.id} to recently edited reminders`);
  ...
  useEffect(() => {
    ...
    completedTodoIds.forEach((id) => {
      ...
      console.log(`🧹 Removed task ${id} from recently edited reminders (completed)`);
