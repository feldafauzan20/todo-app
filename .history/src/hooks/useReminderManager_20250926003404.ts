// hooks/useReminderManager.ts - FIX parsing mirip deadline
    ...
    const scheduleReminders = () => {
      ...
      todos.forEach((todo) => {
        if (
          ...
          // ✅ FIX: Parse reminder time sama seperti deadline
          // Convert UTC datetime ke WIB (sama seperti overdue manager)
          const reminderTime = new Date(todo.reminder + (todo.reminder.includes('T') ? '' : 'T00:00'));
          
          // ✅ Add WIB offset +7 hours (sama seperti deadline logic)
          reminderTime.setHours(reminderTime.getHours() + 7);
          
          // ✅ DEBUG: Log actual times
          console.log(`Current time: ${now.toISOString()}`);
          console.log(`Reminder time (WIB converted): ${reminderTime.toISOString()}`);
          ...
          if (timeUntilReminder <= -60000) {
            console.log(
              `Reminder for "${todo.text}" already passed (more than 1 minute ago)`
            );
            return;
          }
