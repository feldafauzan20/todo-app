      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            if (todo.reminder.includes('+00:00')) {
              // Jika reminder sudah ada +00:00, ini artinya disimpan sebagai UTC
              // Tapi user inputnya adalah waktu WIB, jadi kita perlu convert
              const utcTime = new Date(todo.reminder);
              // Convert UTC kembali ke waktu yang dimaksud user (WIB)
              // User input jam 22:26 WIB, tapi tersimpan sebagai 22:26 UTC
              // Jadi kita perlu menambah 7 jam untuk mendapatkan waktu WIB yang benar
              reminderTime = new Date(utcTime.getTime() + (7 * 60 * 60 * 1000));
              console.log(`[REMINDER DEBUG] Converted UTC back to intended WIB time`);
            } else {
