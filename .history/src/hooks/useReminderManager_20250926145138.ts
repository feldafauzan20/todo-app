// hooks/useReminderManager.ts - FIX support edit reminder
...
export const useReminderManager = (
  ...
  useEffect(() => {
    ...
    const scheduleReminders = () => {
      ...
      console.log(`📋 Current reminders map:`, Object.fromEntries(currentReminders));
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          console.log(`🔍 Current reminder for todo ${todo.id}: ${currentReminder}`);
          console.log(`🔍 New reminder: ${todo.reminder}`);
          ...
          // ✅ Update current reminder tracking
          currentReminders.set(todo.id, todo.reminder);

          // ✅ CHECK: Skip if this exact reminder already processed
          ...
          console.log(`⏳ Time until reminder: ${timeUntilReminder}ms (${minutesUntil} minutes)`);

          if (timeUntilReminder <= -60000) {
            console.log(`⏭️ SKIPPING - Reminder too old`);
            return;
          }
          ...
          if (timeUntilReminder > 0) {
            ...
            console.log(`✅ Timer created for todo ${todo.id} in ${minutesUntil} minutes`);
      ...
      console.log(`📋 Final current reminders:`, Object.fromEntries(currentReminders));
      console.log(`📋 Final processed reminders:`, Array.from(processedReminders));
    };
