...
export const useReminderManager = (
  ...
  useEffect(() => {
    ...
    logger.log(`useEffect triggered (${effectCallCount.current})`);
      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          if (currentReminder && currentReminder !== todo.reminder) {
            logger.info(`Reminder time changed for task ${todo.id}`);
          ...
          try {
            ...
            if (isNaN(utcTime.getTime())) {
              logger.error(`Invalid reminder time for task ${todo.id}: ${todo.reminder}`);
              return;
            }
                  ...
                  showToast(
                    ...
                    `Don't forget: ${todo.text}`,
                    {
                      ...
                      onClick: () => {
                        ...
                        logger.info(`Mark done clicked for task ${todo.id}`);
                      },
                    }
                  );
                }
            ...
            logger.error(`Error processing reminder for task ${todo.id}:`, error);
          }
        }
      });
    };
