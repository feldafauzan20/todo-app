      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            console.log(`[REMINDER DEBUG] Current UTC time:`, now.toISOString());
            console.log(`[REMINDER DEBUG] Current WIB time:`, nowWIB.toISOString());
            console.log(`[REMINDER DEBUG] Task ${todo.id} reminder:`, todo.reminder);
            
            // Parse reminder time - assume input is in WIB timezone
            let reminderTime: Date;
            
            if (todo.reminder.includes('T') && (todo.reminder.includes('+') || todo.reminder.includes('Z'))) {
              // Already has timezone info - use as is
              reminderTime = new Date(todo.reminder);
              console.log(`[REMINDER DEBUG] Reminder has timezone info`);
            } else {
              // Input is treated as local WIB time, need to handle properly
              // If it's in format "2025-09-26 22:26:00", treat as WIB
              const localTime = new Date(todo.reminder);
              
              // Check if the parsed time is valid
              if (!isNaN(localTime.getTime())) {
                // Assume this local time is in WIB timezone
                // To convert WIB to UTC: subtract 7 hours
                reminderTime = new Date(localTime.getTime() - (7 * 60 * 60 * 1000));
                console.log(`[REMINDER DEBUG] Converted WIB input to UTC`);
              } else {
                console.error(`[REMINDER DEBUG] Invalid date format: ${todo.reminder}`);
                return;
              }
            }
            
            console.log(`[REMINDER DEBUG] Parsed reminder time (UTC):`, reminderTime.toISOString());
            console.log(`[REMINDER DEBUG] Parsed reminder time (WIB):`, new Date(reminderTime.getTime() + (7 * 60 * 60 * 1000)).toISOString());

            if (isNaN(reminderTime.getTime())) {
              console.error(`Invalid reminder time for task ${todo.id}: ${todo.reminder}`);
              return;
            }

            const delay = reminderTime.getTime() - now.getTime();
            console.log(`[REMINDER DEBUG] Delay in ms:`, delay, `(${Math.round(delay/1000)} seconds)`);

            if (delay > 0 && delay < 24 * 60 * 60 * 1000) {
              // Only schedule if within 24 hours
              console.log(`[REMINDER DEBUG] Scheduling reminder for task ${todo.id} in ${Math.round(delay/1000)} seconds`);
              ...
              console.log(`[REMINDER DEBUG] Reminder is recent (${Math.round(delay/1000)}s ago), showing immediately for task ${todo.id}`);
              ...
              console.log(`[REMINDER DEBUG] Reminder is too old or too far (${Math.round(delay/1000)}s), skipping task ${todo.id}`);
            }
          } catch (error) {
            console.error(
