    ...
    const checkOverdueAndNotify = () => {
      ...
      memoizedTodos.forEach((todo) => {
        ...
        if (
          ...
          // Handle UTC format - convert to local time
          const dateTimeString = todo.deadline
            .replace("+00:00", "")
            .replace(".000Z", "");
          deadlineDate = new Date(dateTimeString);
          console.log(`[OVERDUE DEBUG] Task ${todo.id} deadline: ${todo.deadline} -> parsed as local: ${deadlineDate.toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}`);
        } else {
          // Treat as local time
          deadlineDate = new Date(todo.deadline);
          console.log(`[OVERDUE DEBUG] Task ${todo.id} deadline treated as local: ${deadlineDate.toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}`);
        }
        ...
        console.log(`[OVERDUE DEBUG] Task ${todo.id} overdue threshold: ${overdueDeadline.toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}, current: ${nowInWIB.toLocaleString("id-ID", { timeZone: "Asia/Jakarta" })}`);
        ...
        if (isCurrentlyOverdue) {
          ...
          if (!notifiedTasksRef.current.has(todo.id)) {
            ...
            console.log(`[OVERDUE DEBUG] Task ${todo.id} is NEWLY overdue - will trigger notification!`);
          } else {
      ...
      if (isNotificationEnabled && newlyOverdue.length > 0) {
        console.log(`[OVERDUE DEBUG] Triggering notifications for ${newlyOverdue.length} newly overdue tasks`);
        newlyOverdue.forEach((todo) => {
          console.log(`[OVERDUE DEBUG] Calling showOverdueNotification for task ${todo.id}: ${todo.text}`);
        ...
        if (!isNotificationEnabled) {
          console.log(`[OVERDUE DEBUG] Notifications disabled`);
        }
        if (newlyOverdue.length === 0) {
          console.log(`[OVERDUE DEBUG] No newly overdue tasks`);
        }
      }
    };
