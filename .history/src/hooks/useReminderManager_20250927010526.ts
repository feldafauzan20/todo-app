      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            if (
              todo.reminder.includes("+00:00") ||
              todo.reminder.endsWith(".000Z")
            ) {
              ...
              const dateTimeString = todo.reminder
                .replace("+00:00", "")
                .replace(".000Z", "");
              reminderTime = new Date(dateTimeString);
              console.log(
                `[REMINDER DEBUG] Parsed UTC format (${
                  todo.reminder.includes("+00:00") ? "+00:00" : ".000Z"
                }) as local WIB time`
              );
            } else {
