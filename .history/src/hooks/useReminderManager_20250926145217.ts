    ...
    const scheduleReminders = () => {
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          const currentReminder = currentReminders.get(todo.id); // "12:00"
          const newReminder = todo.reminder; // "13:00" (after edit)

          console.log(`🔑 Generated key: ${reminderKey}`);
          console.log(
            `🔍 Current reminder for todo ${todo.id}: ${currentReminder}`
          );
          ...
          if (currentReminder && currentReminder !== todo.reminder) {
            ...
            processedReminders.delete(oldKey); // Remove old processed flag
          ...
          console.log(
            `⏳ Time until reminder: ${timeUntilReminder}ms (${minutesUntil} minutes)`
          );
