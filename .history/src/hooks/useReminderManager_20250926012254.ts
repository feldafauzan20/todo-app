...
export const useReminderManager = (
  ...
  // ✅ ADD: Track toast notifications to prevent duplicates
  const toastNotifications = useRef(new Set<number>());
  ...
  useEffect(() => {
    const scheduleReminders = () => {
      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          // Check if reminder time changed
          ...
          if (previousReminderTime && previousReminderTime !== todo.reminder) {
            console.log(
              `📝 Reminder time changed for "${todo.text}", clearing notification flags`
            );
            notifiedReminders.current.delete(todo.id);
            // ✅ FIX: Also clear toast notification flag
            toastNotifications.current.delete(todo.id);
          }

          // Store current reminder time
          previousReminders.current.set(todo.id, todo.reminder);

          // ✅ FIX: Skip if already notified OR already toasted
          if (
            notifiedReminders.current.has(todo.id) ||
            toastNotifications.current.has(todo.id)
          ) {
            console.log(
              `⏭️ Reminder for "${todo.text}" already processed, skipping`
            );
            return;
          }
          ...
          if (timeUntilReminder > 0) {
            const timer = setTimeout(() => {
              ...
              // ✅ FIX: Double-check before showing notification
              if (!toastNotifications.current.has(todo.id)) {
                showReminderNotification(todo);
                notifiedReminders.current.add(todo.id);
                toastNotifications.current.add(todo.id); // ✅ ADD: Mark as toasted
              } else {
                console.log(
                  `🚫 Toast already shown for "${todo.text}", skipping`
                );
              }
    ...
    const showReminderNotification = async (todo: Todo) => {
      try {
        ...
        // if (showToast && !toastNotifications.current.has(todo.id)) {
        ...
        //     }
        //   );
        //   toastNotifications.current.add(todo.id); // ✅ Mark as toasted
        // } else if (toastNotifications.current.has(todo.id)) {
        //   console.log(`🚫 Toast already shown for: "${todo.text}"`);
        // }
    ...
    // ✅ FIX: Increase interval to reduce duplicate checks
