      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            console.log(`[REMINDER DEBUG] Current time:`, now.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' }));
            console.log(`[REMINDER DEBUG] Task ${todo.id} reminder:`, todo.reminder);
            
            // Parse reminder time - treat as local time
            const reminderTime = new Date(todo.reminder);
            
            console.log(`[REMINDER DEBUG] Parsed reminder time:`, reminderTime.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' }));
            ...
            if (delay > 0 && delay < 24 * 60 * 60 * 1000) {
              ...
              const timer = setTimeout(() => {
                console.log(`[REMINDER DEBUG] ⏰ REMINDER TRIGGERED for task ${todo.id} at ${new Date().toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
