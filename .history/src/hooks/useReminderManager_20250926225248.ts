      ...
      todos.forEach((todo) => {
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            if (todo.reminder.includes("+00:00") || todo.reminder.endsWith(".000Z")) {
              // Jika reminder berformat +00:00 atau .000Z, artinya ini UTC format
              // yang sebenarnya dimaksudkan sebagai waktu lokal WIB
              // Contoh: user input 22:26 WIB -> tersimpan "2025-09-26T22:26:00+00:00" atau "2025-09-26T22:26:00.000Z"
              // Kita ambil bagian tanggal dan jam saja, lalu parse sebagai waktu lokal
              const dateTimeString = todo.reminder.replace("+00:00", "").replace(".000Z", "");
              reminderTime = new Date(dateTimeString);
              console.log(
                `[REMINDER DEBUG] Parsed UTC format (${todo.reminder.includes("+00:00") ? "+00:00" : ".000Z"}) as local WIB time`
              );
            } else {
              // Parse reminder time - treat as local time
              reminderTime = new Date(todo.reminder);
              console.log(`[REMINDER DEBUG] Treating as local time`);
            }
            ...
            if (delayMs > 0 && delayMs < 24 * 60 * 60 * 1000) {
              ...
              console.log(
                `[REMINDER DEBUG] Scheduling reminder for task ${
                  todo.id
                } in ${Math.round(delayMs / 1000)} seconds`
              );
            ...
            } else if (delayMs > -60000 && delayMs <= 0) {
              console.log(
