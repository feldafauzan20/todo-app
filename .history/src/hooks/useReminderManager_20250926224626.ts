        ...
        if (!todo.is_done && todo.reminder) {
          ...
          try {
            ...
            if (todo.reminder.includes("+00:00")) {
              // Jika reminder berformat +00:00, artinya ini dari datetime-local input
              // yang sudah dikonversi ke UTC. Kita perlu parse sebagai waktu lokal.
              // Contoh: user input 22:26 WIB -> tersimpan "2025-09-26T22:26:00+00:00"
              // Kita ambil bagian tanggal dan jam saja, lalu parse sebagai waktu lokal
              const dateTimeString = todo.reminder.replace("+00:00", "");
              reminderTime = new Date(dateTimeString);
              console.log(
                `[REMINDER DEBUG] Parsed +00:00 format as local WIB time`
              );
            } else {
