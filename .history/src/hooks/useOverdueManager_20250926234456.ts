    ...
    const checkOverdueAndNotify = () => {
      ...
      memoizedTodos.forEach((todo) => {
        ...
        if (previousDeadline && previousDeadline !== todo.deadline) {
          ...
          console.log(`[OVERDUE DEBUG] Task ${todo.id} deadline changed from ${previousDeadline} to ${todo.deadline} - reset notification state`);
        }
        ...
        if (
          ...
          console.log(
            `[OVERDUE DEBUG] Task ${todo.id} deadline: ${
              todo.deadline
            } -> parsed as local: ${deadlineDate.toLocaleString("id-ID", {
      ...
      if (isNotificationEnabled && newlyOverdue.length > 0) {
        console.log(
          `[OVERDUE DEBUG] Triggering notifications for ${newlyOverdue.length} newly overdue tasks`
        );
        newlyOverdue.forEach((todo) => {
          console.log(
            `[OVERDUE DEBUG] Calling showOverdueNotification for task ${todo.id}: ${todo.text}`
          );
