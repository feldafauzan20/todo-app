...
export const useReminderManager = (
  ...
  useEffect(() => {
    ...
    const scheduleReminders = () => {
      ...
      console.log(`⏰ Current LOCAL time: ${now.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          if (reminderTimeString.includes("+00:00")) {
            ...
            console.log(`🌍 Input format: UTC timezone (${reminderTimeString})`);
            
            // Parse sebagai UTC
            const utcTime = new Date(reminderTimeString);
            console.log(`🌍 Parsed UTC time: ${utcTime.toISOString()}`);
            console.log(`🌍 UTC as WIB: ${utcTime.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
            ...
            console.log(`🌍 CORRECTED: Treated as LOCAL time: ${reminderTime.toISOString()}`);
            console.log(`🌍 CORRECTED: Local display: ${reminderTime.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
            ...
            const cleanTimeString = reminderTimeString.replace("Z", "").replace(".000", "");
            ...
            console.log(`🌍 Input format: Local timezone (${reminderTimeString})`);
          ...
          console.log(`🕐 Final reminder time (UTC): ${reminderTime.toISOString()}`);
          console.log(`🕐 Final reminder time (WIB): ${reminderTime.toLocaleString('id-ID', { timeZone: 'Asia/Jakarta' })}`);
          ...
          if (timeUntilReminder > 0) {
            ...
            console.log(`✅ Timer created for todo ${todo.id} in ${minutesUntil} minutes (${secondsUntil} seconds)`);
          }
        } else {
          // Cleanup
  ...
  useEffect(() => {
    const completedTodoIds = todos.filter((todo) => todo.is_done).map((todo) => todo.id);
