...
export const useReminderManager = (
  ...
  // ✅ NEW: Function untuk manually clear reminded task effect
  const clearReminderEffect = (todoId: number) => {
    console.log(`🧹 [MANUAL CLEAR] Manually clearing reminder effect for task ${todoId}`);
    ...
    if (remindedCleanupTimers.current.has(todoId)) {
      ...
      console.log(`🧹 [MANUAL CLEAR] Cleared cleanup timer for task ${todoId}`);
    }
  };
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          if (currentReminder && currentReminder !== todo.reminder) {
            ...
            // ✅ FIX: CLEAR visual effects when reminder changed (remove pulse immediately)
            setRemindedTasks((prev) => {
              const newSet = new Set(prev);
            ...
            console.log(
              `🧹 CLEARED visual effects for task ${todo.id} (reminder changed)`
            );
            ...
            if (remindedCleanupTimers.current.has(todo.id)) {
              ...
              console.log(`🧹 Cleared cleanup timer for task ${todo.id}`);
            }
          }
  ...
  useEffect(() => {
    ...
    completedTodoIds.forEach((id) => {
      ...
      // Visual effects cleared when task is completed
  ...
  // ✅ Return reminded tasks and clear function
  return {
    remindedTasks: Array.from(remindedTasks), // Convert Set ke Array untuk easier usage
    clearReminderEffect, // ✅ NEW: Function untuk manually clear reminder effect
  };
};
