    ...
    const scheduleReminders = () => {
      ...
      todos.forEach((todo, index) => {
        ...
        if (!todo.is_done && todo.reminder) {
          ...
          console.log(`🔍 [EFFECT #${effectCallCount.current}] Key exists in GLOBAL processed: ${globalProcessedKeys.has(reminderKey)}`);

          // ✅ FIX: Check global processed keys
          if (globalProcessedKeys.has(reminderKey)) {
            console.log(`⏭️ [EFFECT #${effectCallCount.current}] SKIPPING - Already processed globally: ${reminderKey}`);
            return;
          }
          ...
          if (timeUntilReminder <= -60000) {
            console.log(`⏭️ [EFFECT #${effectCallCount.current}] SKIPPING - Reminder too old`);
            return;
          }
          ...
          if (timeUntilReminder > 0) {
            ...
            console.log(`✅ [EFFECT #${effectCallCount.current}] ADDED to global keys: ${reminderKey}`);
            console.log(`📋 [EFFECT #${effectCallCount.current}] Global keys after add:`, Array.from(globalProcessedKeys));
      ...
      console.log(`📋 [EFFECT #${effectCallCount.current}] Final global keys:`, Array.from(globalProcessedKeys));
    };
