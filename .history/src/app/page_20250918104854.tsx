...
export default function LandingPage() {
  ...
  // Motion values untuk cursor
  const mouseX = useMotionValue(0);
  const mouseY = useMotionValue(0);

  // Smooth movement pakai spring
  const springX = useSpring(mouseX, { stiffness: 50, damping: 20 });
  const springY = useSpring(mouseY, { stiffness: 50, damping: 20 });

  // Scroll-based motion
  const { scrollY } = useScroll();
  const scrollOffset = useTransform(scrollY, [0, 500], [0, 100]); // range scroll → gerak blob

  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      const { innerWidth, innerHeight } = window;
      // Normalisasi posisi cursor -0.5 sampai 0.5
      const x = e.clientX / innerWidth - 0.5;
      const y = e.clientY / innerHeight - 0.5;
      mouseX.set(x * 100); // atur range gerakan blob
      mouseY.set(y * 100);
    };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-purple-50 via-pink-50 to-purple-100">
      ...
      <div className="relative h-screen overflow-hidden">
        {/* Parallax background (mesh gradient) */}
        <motion.div
        ...
        {/* Floating blobs (mouse + scroll reactive) */}
        <motion.div
          style={{ x: springX, y: springY, translateY: scrollOffset }}
          className="absolute top-20 left-20 w-64 h-64 bg-purple-300/40 rounded-full blur-3xl"
        />
        <motion.div
          style={{ x: springX, y: springY, translateY: scrollOffset }}
          className="absolute bottom-20 right-32 w-72 h-72 bg-pink-300/40 rounded-full blur-3xl"
        />
