...
export default function Home() {
  ...
  useEffect(() => {
    const checkUser = async () => {
      ...
      if (!session?.user) {
        router.replace("/login"); // redirect ke login kalau belum login
      } else {
        setUser(session.user);
        fetchTodos(); // load todo kalau sudah login
      }
    };
    checkUser();
  }, []);
  ...
  const handleLogout = async () => {
    ...
    else window.location.href = "/login"; // redirect ke halaman login
  };
